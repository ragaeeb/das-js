...
import { daily, isFard } from '@/utils/calculator';
import hijri from '@/utils/hijri';

const getLabel = (_event: string, label: string, onClick?: string, link?: string) => {
  if (onClick && link) {
    return (
...
const renderSunnah = (event: string, label: string, time?: string) =>
  time && (
...
const renderTiming =
  ...
  ({ label, time, iqamah, event }: TimingProps) => {
    if (!isFard(event)) {
